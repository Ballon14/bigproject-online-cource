    ...
    <div class="p-4 md:p-6 pt-0">
        ...
        <div class="bg-white shadow-lg rounded-xl p-6 mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <div class="border border-gray-200 rounded-lg p-4 text-center">
                    ...
                    <div class="text-2xl font-bold text-yellow-600">{{ number_format($bobot['rating'], 2) }}%</div>
                </div>
                <div class="border border-gray-200 rounded-lg p-4 text-center">
                ...
                <div class="border border-gray-200 rounded-lg p-4 text-center">
                    ...
                    <div class="text-2xl font-bold text-green-600">{{ number_format($bobot['fleksibilitas'], 2) }}%
                    </div>
                </div>
                <div class="border border-gray-200 rounded-lg p-4 text-center">
                    <div class="text-sm text-gray-600 mb-1">Certificate</div>
                    <div class="text-2xl font-bold text-purple-600">{{ number_format($bobot['sertifikat'], 2) }}%</div>
                </div>
                <div class="border border-gray-200 rounded-lg p-4 text-center">
                    <div class="text-sm text-gray-600 mb-1">Last Update</div>
                    <div class="text-2xl font-bold text-orange-600">{{ number_format($bobot['update_terakhir'], 2) }}%
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <td class="px-4 py-4 text-center border border-gray-200">
                                    @if ($result->ranking <= 3)
                                        <span
                                            ...
                                            aria-label="Ranking {{ $result->ranking }}">
                                            {{ $result->ranking }}
                                        </span>
                                    @else
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                ...
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->course->biaya, 0) }}K</span>
                            </div>
                            <div>
                                <span class="text-gray-500 block mb-1">Rating</span>
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->course->rating, 1) }}</span>
                            </div>
        ...
        <div class="bg-white shadow-lg rounded-xl p-6 mb-6">
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center border border-gray-200 font-bold text-gray-700"
                                scope="col">Duration</th>
                            <th class="px-4 py-3 text-center border border-gray-200 font-bold text-gray-700"
                                scope="col">Flexibility</th>
                            <th class="px-4 py-3 text-center border border-gray-200 font-bold text-gray-700"
                                scope="col">Certificate</th>
                            <th class="px-4 py-3 text-center border border-gray-200 font-bold text-gray-700"
                                scope="col">Last Update</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($results as $result)
                            <tr class="{{ $result->ranking % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                                ...
                                <td class="px-4 py-3 border border-gray-200 text-center">
                                    {{ number_format($result->norm_biaya, 4) }}</td>
                                <td class="px-4 py-3 border border-gray-200 text-center">
                                    {{ number_format($result->norm_rating, 4) }}</td>
                                <td class="px-4 py-3 border border-gray-200 text-center">
                                    {{ number_format($result->norm_durasi, 4) }}</td>
                                <td class="px-4 py-3 border border-gray-200 text-center">
                                    {{ number_format($result->norm_fleksibilitas, 4) }}</td>
                                <td class="px-4 py-3 border border-gray-200 text-center">
                                    {{ number_format($result->norm_sertifikat, 4) }}</td>
                                <td class="px-4 py-3 border border-gray-200 text-center">
                                    {{ number_format($result->norm_update, 4) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="lg:hidden space-y-4">
                @foreach ($results as $result)
                    <div
                        class="border border-gray-200 rounded-lg p-4 {{ $result->ranking % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                        <div class="flex items-center gap-2 mb-3">
                            <span
                                class="text-xs font-bold text-indigo-600 bg-indigo-100 px-2 py-1 rounded">#{{ $result->ranking }}</span>
                            <h4 class="font-semibold text-gray-900 text-sm flex-1 truncate">
                                {{ $result->course->nama_kursus }}</h4>
                        </div>
                        <div class="grid grid-cols-2 gap-2 text-xs">
                            <div>
                                ...
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->norm_biaya, 4) }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 block">Rating</span>
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->norm_rating, 4) }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 block">Duration</span>
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->norm_durasi, 4) }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 block">Flexibility</span>
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->norm_fleksibilitas, 4) }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 block">Certificate</span>
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->norm_sertifikat, 4) }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 block">Last Update</span>
                                <span
                                    class="font-semibold text-gray-900">{{ number_format($result->norm_update, 4) }}</span>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="flex flex-col sm:flex-row gap-4">
            <a href="{{ route('perhitungan') }}"
                ...
                aria-label="Recalculate SAW calculation">
                Recalculate SAW
            </a>
            <a href="{{ route('dashboard') }}"
                ...
                aria-label="Back to dashboard page">
                Back to Dashboard
            </a>
        </div>
    </div>
