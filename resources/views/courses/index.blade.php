        ...
        <div class="hidden lg:block bg-white shadow rounded-xl py-2 px-2 md:px-6 md:py-4 overflow-x-auto">
            <table class="min-w-full border divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase" scope="col">
                            <a
                                href="{{ route('courses.index', ['search' => $search, 'sort' => 'id', 'direction' => $sortField === 'id' && $sortDirection === 'asc' ? 'desc' : 'asc']) }}">
                                # @if ($sortField === 'id')
                                    {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                @endif
                            </a>
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase" scope="col">
                            <a
                                href="{{ route('courses.index', ['search' => $search, 'sort' => 'nama_kursus', 'direction' => $sortField === 'nama_kursus' && $sortDirection === 'asc' ? 'desc' : 'asc']) }}">
                                Course Name @if ($sortField === 'nama_kursus')
                                    {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                @endif
                            </a>
                        </th>
                        <th class="px-4 py-3 text-xs font-bold text-gray-700 uppercase" scope="col">
                            <a
                                href="{{ route('courses.index', ['search' => $search, 'sort' => 'biaya', 'direction' => $sortField === 'biaya' && $sortDirection === 'asc' ? 'desc' : 'asc']) }}">
                                Cost @if ($sortField === 'biaya')
                                    {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                @endif
                            </a>
                        </th>
                        <th class="px-4 py-3 text-xs font-bold text-gray-700 uppercase" scope="col">
                            <a
                                href="{{ route('courses.index', ['search' => $search, 'sort' => 'rating', 'direction' => $sortField === 'rating' && $sortDirection === 'asc' ? 'desc' : 'asc']) }}">
                                Rating @if ($sortField === 'rating')
                                    {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                @endif
                            </a>
                        </th>
            ...
            @forelse ($courses as $i => $c)
                <div class="bg-white shadow rounded-xl p-4 hover:shadow-md transition-shadow">
                    ...
                    <div class="flex gap-2 pt-3 border-t border-gray-200">
                        ...
                        <form action="{{ route('courses.destroy', $c->id) }}" method="POST" class="flex-1"
                            onsubmit="return confirm('Are you sure you want to delete this course?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
